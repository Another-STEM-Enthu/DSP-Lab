 ...
 main( void )
{
    ...
    Int16 outputData[100];//array for storing output samples to be viewed in graphing window
    int i=0;	
    int sample = 0;
	/* Initialize BSL */
    SYS_EXBUSSEL = 0x6100;
	...
	while(TRUE)
    {
		...
		{
			while((Xmit & I2S2_IR)==0); // Wait for transmit interrupt to be pending
			I2S2_W0_MSW_W=(sinetable[sample]);
			I2S2_W1_MSW_W=(sinetable[sample]);
			outputData[i]=sinetable[sample];
			i=(i+1)%100;
		}
    }   
    /* Disble I2S */
