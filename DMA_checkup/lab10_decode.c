...
Int16 max_val1, max_val2, max_idx1, max_idx2;
	...
	while (1)
	{
		...
		do_fft(BufferL, realpart_fft, imagpart_fft, scale);
		 ...
		 for(i=0;i<FFT_LENGTH/2;i++){
			abs_fft[i] = ((_lsmpy(realpart_fft[i], realpart_fft[i]) + _lsmpy(imagpart_fft[i], imagpart_fft[i]))>>1)/100;
			if (i<32){
						    if (abs_fft[i] > max_val1){
						    	max_val1 = abs_fft[i];
						        max_idx1 = i;
						    }
					    }
					    if (i>32){
						    if (abs_fft[i] > max_val2){
						    	max_val2 = abs_fft[i];
						        max_idx2 = i;
						    }
					    }
...
//		freq1 = int(float(max_idx1)*31.25);    
//		freq2 = int(float(max_idx2)*31.25);
		freq1 = max_idx1*31.25;
		freq2 = max_idx2*31.25;
if(max_val1 < 500){
			freq1 = 0;
		}
		if(max_val2 < 1000){
		...
		if (freq1 >670 && freq1 <720){
		   if (freq2 > 1190 && freq2 < 1250){
		   	key = '1';
		   }
		   else if (freq2 > 1290 && freq2 < 1400){
		   	key = '2';
		   }
		   else if (freq2 > 1400 && freq2 < 1530){
		   	key = '3';
		   }
		   else{key = 'N';}
		}
		else if (freq1 >720 && freq1 <810){
		   if (freq2 > 1190 && freq2 < 1250){
		   	key = '4';
		   }
		   else if (freq2 > 1290 && freq2 < 1400){
